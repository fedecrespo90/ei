var DB, Everyone
  ...
  , Asignacion = function(db, everyone) {
  DB = db;
  Everyone = everyone;
...
Asignacion.get = function(req, res, next) {
...
Asignacion.post = function(req, res, next) {
          ...
          imp.forEach(function(i){
            if(req.body[i.nombre] != ''){
              ...
              DB.CronogramaImpuesto.create({
                ...
                impuesto_id: i.id,
                vtoAsignacion: moment(req.body[i.nombre]).format('YYYY-MM-DD HH:MM:SS') 
              })
            }
          })
          res.send(true);
          ...
          imp.forEach(function(i){
            if(req.body[i.nombre] != ''){
              ...
              DB.CronogramaImpuesto.find({where:{impuesto_id:i.id, cronograma_id:crono.id}}).on('success', function(cronoImpuesto){
                if(cronoImpuesto){
                    cronoImpuesto.updateAttributes({vtoAsignacion: req.body[i.nombre]}).success(function(){res.send(true)})                  
                }else{
                  DB.CronogramaImpuesto.create({cronograma_id: crono.id, impuesto_id: i.id, vtoAsignacion: moment(req.body[i.nombre]).format('YYYY-MM-DD HH:MM:SS') }).on('success', function(){res.send(true)})
                }              
              })                
            }
          })
...
Asignacion.put = function(req, res, next) {
        ...
        DB.Impuesto.findAll().on('success', function(imp){
          imp.forEach(function(i){
            if(req.body[i.nombre] != ''){
              ...
              DB.CronogramaImpuesto.find({where:{impuesto_id:i.id, cronograma_id:crono.id}}).on('success', function(cronoImpuesto){
                if(cronoImpuesto){
                    cronoImpuesto.updateAttributes({vtoAsignacion: req.body[i.nombre]}).success(function(){
                      res.send(true)
                    })                  
                }else{
                  DB.CronogramaImpuesto.create({
...
Asignacion.delete = function(req, res, next) {
