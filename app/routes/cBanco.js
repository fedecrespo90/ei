...
var CBanco = function(db, everyone) {
  DB = db;
  Everyone = everyone;
  return CBanco;
};

CBanco.get = function(req, res, next) {
  DB.MovimientoCaja.findAll({where: {chequeado:1, caja_id:3}, include:[{model: DB.Caja}]}).on('success', function(mov){
    var msg=[]
