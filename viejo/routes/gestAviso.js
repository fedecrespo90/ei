var DB, Everyone
  ...
  , GestAviso = function(db, everyone) {
  DB = db;
  Everyone = everyone;
...
GestAviso.get = function(req, res, next) {
...
GestAviso.post = function(req, res, next) {
  res.send(true)
  DB.AsigPie.find({where:{ id:1}}).on('success', function(asigPie){
    ...
    DB.Cliente.findAll({include:[{model: DB.ClienteRubro, where:{rubro_id: req.body.rubro_id}}]}).on('success', function(clientes){
      /*DB.Vencimiento.findAll({order:'cliente_id DESC', where:{ adeudado:1, pagado:0}, include:[{model: DB.ClienteImpuesto},{model:DB.Impuesto},
        {model: DB.CronogramaImpuesto, order:'vencimiento  ASC', include:[{model: DB.Cronograma}]},{model: DB.Cliente, include: [{model: DB.ClienteRubro, where:{rubro_id:req.body.rubro_id }}]}]}).on('success', function(vencimientos){//si pusiera los pagados. 
        */
      DB.Vencimiento.findAll({where:{ adeudado:1, pagado:0}, include:[{model: DB.ClienteImpuesto},{model:DB.Impuesto},
        {model: DB.CronogramaImpuesto, include:[{model: DB.Cronograma}]},{model: DB.Cliente, include: [{model: DB.ClienteRubro, where:{rubro_id:req.body.rubro_id }}]}],
        order: "`cronogramaImpuesto`.`vtoImpuesto` ASC"
        }).on('success', function(vencimientos){//si pusiera los pagados. 

        var puedo = true;
        //Reviso si puedo ordenar los vtos
        ...
        //SI PUEDO Ordeno los vtos por fecha.
        if(puedo)
         ...
         //else //COMENTE EL ELSE 
          // Tengo qe iterar, pero va a haber distintos clientes, (
          // por cada cliente tengo que juntar todos sus impuestos, y ir guardandolos en variables, cuando viene un cliente nuevo , agarro la lista de los impuesto que tiene este cliente y agrego a la lista de los impuesto de ese cliente y sacudo mail  ) 
         clientes.forEach(function(cliente){
          ...
          //BUSCO MATCH PARA CREAR EL MAIL
            if(cliente.id==vencimiento.cliente.id){
              if(vencimiento.adeudado!=1 && vencimiento.pagado!=0 && vencimiento.cronograma_id == req.body.cronograma_id)//&& vencimiento.cronograma_id == req.body.cronograma_id no estaba
